...
void testApp::draw(){
    ...
    for (int i = 0; i < numUsers; i++){
       ...
       dist_time=current_time-prev_time;
        if(dist_time>100){switchT = true;}
       if(switchT){prev_time = ofGetElapsedTimef(); prev_left_hand=left_hand; switchT=false;}
       float dist_left=ofDist(left_hand.x, left_hand.y, prev_left_hand.x, prev_left_hand.y);
        
        CGPoint point;
        point.x = left_hand.x-50;
        point.y = left_hand.y-50;
        CGWarpMouseCursorPosition(point);
        
        
        
      //判断拉拽行为
        if(dist_left>30&&dist_time<100){
            bool left_drag=true;
            ofSetColor(255,0,0);
            ofCircle(20, 20, 20);
